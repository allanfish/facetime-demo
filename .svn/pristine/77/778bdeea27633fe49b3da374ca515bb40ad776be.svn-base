...
public class MySQLUserDaoImpl implements UserDao {
	...
	public MySQLUserDaoImpl(DatabaseType dbType) {
		this.dbType = dbType;
	}
	...
	public void createTable() {
		...
		try {
			...
			ps = con.prepareStatement("create table user (id int(4) primary key auto_increment, name varchar(40))");
	...
	public void deleteUser(User user) {
		...
		try {
			...
			con.setAutoCommit(false);
			ps = con.prepareStatement("delete from user where user.id = ? and user.name = ?");
			ps.setInt(1, user.getId());
			ps.setString(2, user.getName());
			ps.executeUpdate();
			System.out.println("delete user by id:" + user.getId());
			con.commit();
		} catch (Exception e) {
			try {
				con.rollback();
	...
	public List<User> getUserByName(String name) {
		...
		try {
			ps = con.prepareStatement("select id,name from user where user.name = ?");
			ps.setString(1, name);
			rs = ps.executeQuery();
	...
	public void save(User user) {
		...
		try {
			con.setAutoCommit(false);
			ps = con.prepareStatement("insert into user(name) values(?)");
