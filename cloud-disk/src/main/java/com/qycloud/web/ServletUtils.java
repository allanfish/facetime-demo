...
/**
 * Copy from GWT RPCServletutils, <br>
...
public class ServletUtils {
	 ...
	 *            the expected content (i.e. 'type/subtype' only) in the
	 *            Content-Type request header, or <code>null</code> if no
	 *            validation is to be performed, and you are willing to allow
	 *            for some types of cross type security attacks
	 * @param expectedCharSet
	 *            the expected request charset, or <code>null</code> if no
	 *            charset validation is to be performed and <code>UTF-8</code>
	 *            should be assumed
	 * @return the content of an {@link HttpServletRequest} by decoding it using
	 *         <code>expectedCharSet</code>, or <code>UTF-8</code> if
	 *         <code>expectedCharSet</code> is <code>null</code>
	 * @throws IOException
	 *             if the request's input stream cannot be accessed, read from
	 *             or closed
	 * @throws ServletException
	 *             if the request's content type does not equal the supplied
	 *             <code>expectedContentType</code> or
	 ...
	 * @param gzipResponse
	 *            if <code>true</code> the response content will be gzip encoded
	 *            before being written into the response
	 * @throws IOException
	 ...
	 * @param expectedCharSet
	 *            the expected charset of the request
	 * @throws ServletException
	 *             if requests encoding is not <code>null</code> and does not
	 *             equal, ignoring case, <code>expectedCharSet</code>
	 */
	private static void checkCharacterEncodingIgnoreCase(HttpServletRequest request, String expectedCharSet)
		...
		if (characterEncoding != null) {
			/*
			 * TODO: It would seem that we should be able to use
			 * equalsIgnoreCase here instead of indexOf. Need to be sure that
			 * servlet engines return a properly parsed character encoding
			 * string if we decide to make this change.
			 */
		...
		if (!encodingOkay) {
			throw new ServletException("Character Encoding is '"
					+ (characterEncoding == null ? "(null)" : characterEncoding) + "'.  Expected '" + expectedCharSet
					+ "'");
		}
	}
	 ...
	 *            the incoming request
	 * @param expectedContentType
	 *            the expected Content-Type for the incoming request
	 * @throws ServletException
	 *             if the request's content type is not <code>null</code> and
	 *             does not, ignoring case, equal
	...
	private static void checkContentTypeIgnoreCase(HttpServletRequest request, String expectedContentType)
		...
		if (contentType != null) {
			 ...
			 * do not remove the charset component but others do.
			 */
