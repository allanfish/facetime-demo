...
public class FileLogic {
	...
	public String cropUserPicture(String postData, String token) throws IOException {
		...
		float imageResizeRatio = ImageUtil.IMAGE_SIZE_96 / imageDTO.getWidth();
		...
		BufferedImage subImage = resizeImage.getSubimage(x, y,
				(int) ImageUtil.IMAGE_SIZE_96, (int) ImageUtil.IMAGE_SIZE_96);
		...
		FileUploadUtil.upload(cropedFile, p, token);
	...
	private long saveFile(SaveFileDTO fileDTO, boolean isNew, String token) throws Exception {
		...
		// 保存html至文件服务器
		FileUploadUtil.upload(htmlFile, htmlPath, token);
