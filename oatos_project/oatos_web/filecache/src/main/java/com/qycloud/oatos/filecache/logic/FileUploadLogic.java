...
public class FileUploadLogic {
	...
	public String screenshot(HttpServletRequest request, final long userId, final String token) throws Exception {
		...
		String re = proxy.post(ConfigUtil.getServiceUrl(RESTurl.screenshot), token, postData);
		String result = null;
		try {
			long fileId = Long.parseLong(re);
			// 保存文件至文件服务器
			FileUploadUtil.upload(filePath, fileId, CommConstants.FILE_TYPE_ONLINEDISK, token);

			// 保存文件缩略图至文件服务器
			if (thumb != null) {
				FileUploadUtil.upload(thumb, CommConstants.FILE_TYPE_PRIVATEFILE_THUMB, token);
			}
	...
	public String tackPictureAndSave(HttpServletRequest request, String postData, final String token) throws Exception {
		...
		String re = proxy.post(ConfigUtil.getServiceUrl(RESTurl.tackPictureAndSave), token, postJson);
		String result = null;
		try {
			long fileId = Long.parseLong(re);
			// 保存文件至文件服务器
			FileUploadUtil.upload(filePath, fileId, pictureDTO.getType(), token);
	...
	public String recordVoiceMail(HttpServletRequest request, String postData, final String token) throws Exception {
		...
		// 保存文件至文件服务器
		FileUploadUtil.upload(filePath, MessageType.VoiceMail, token);
		...
		if (RESTurl.getStartPoint.equals(bean.getMethod())) {
			...
			while (iterator.hasNext()) {
				...
				if (!fis.isFormField()) {
					...
					long size = FileUtil.copyInputStreamToFile(fis.openStream(), tempFile);
					bean.setFileSize(size);
					// 插入记录至数据库
					result = FileUploadUtil.handleUpload(bean, token);
				}
			}
		}
		return result;
